...
@author: sang
This code update Google sheet. 
Use it after joe_f.py
"""
...
dates=[ dd for dd in dates if dd.strip() ]
last_date=datetime.strptime(dates[len(dates)-1], '%B %d, %Y')

Npost_dates=date_sheet.col_values(3)
Npost_dates=[ dd for dd in Npost_dates if dd.strip() ]
StartD=len(Npost_dates)-1
Npost =sum(list(map(int, Npost_dates[1:StartD])))
...
dt_day_list=[datetime.strptime(dl,'%B %d, %Y') for dl in day_list]

update_date=[(dt>=last_date) for dt in dt_day_list ]

for Nday in list(range(StartD,Ndays+1)):
    dp=day_post[Nday-1]
    AggNop=0
    
    if Nday==1:
        print('First Day Error')
    else:
